...
Changes between 1.2c and 1.2b:
- Replaced all output with a report to LuaTestResults.xml in JUnit format

Changes between 1.2b and 1.2a:
...
if dbg.short_src ~= nil then
    print (dbg.short_src)
    RESULT_OUTPUT_FILE = string.sub(dbg.short_src, 0, -5).."-results.xml"
else
--]]
    RESULT_OUTPUT_FILE = "LuaTests-results.xml"
--end
...
--]]
    ...
    function UnitResult:displayOneFailedTest( failure )
		testName, errorMsg = unpack( failure )
		io.write("<testcase name=\"" .. testName .. "\">\n")
		io.write("<failure type=\"Luaunit Assertion\" message=\"" .. xmlchars(errorMsg) .. "\" />\n")
		io.write("</testcase>\n")
	end

	function UnitResult:displayFailedTests()
		-- if table.getn( self.errorList ) == 0 then return end
		io.output(RESULT_OUTPUT_FILE)
		io.write("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n")
		io.write("<testsuite name=\"" .. "\" tests=\"" .. self.testCount .. "\" failures=\"" .. self.failureCount .. "\" errors=\"0\" time=\"0\">\n")
		table.foreachi( self.errorList, self.displayOneFailedTest )
		io.write("</testsuite>\n")
		io.output():close()
	end
...
LuaUnit = {
    ...
    function LuaUnit:runTestMethod(aName, aClassInstance, aMethod)
		...
		if not ok then
			...
			if LOG then LOG("ERROR: "..errorMsg) end
