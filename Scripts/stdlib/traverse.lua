...
debug.getfenv		= debug.getfenv or	function(o)
...
--					"\nvalue:", comefrom[to][i].v)
			print("from: " .. tostring(tfrom) .. 
					"\nhow:" .. tostring(comefrom[to][i].h) ..
					"\nvalue:" .. tostring(comefrom[to][i].v) )
			printpath(tfrom)
		end			
	end
...
function traverse(funcs, ignoreobjs)
	...
	-- ignore functions in this module
	-- env.marked["gc"] = true
	-- env.marked[gc] = true	-- 
	...
	env.marked["traverse"] = true;
	env.marked[countreferences] = true;
	env.marked["countreferences"] = true;
	env.marked[findallpaths] = true;
	env.marked["findallpaths"] = true;
	env.marked[traversetable] = true;
	env.marked["traversetable"] = true;
	env.marked[traversestring] = true;
	env.marked["traversestring"] = true;
	env.marked[traverseuserdata] = true;
	env.marked["traverseuserdata"] = true;
	env.marked[traversefunction] = true;
	env.marked["traversefunction"] = true;
	env.marked[traversethread] = true;
	env.marked["traversethread"] = true;
	...
	-- called traverse
	math.huge = math.huge or math.exp(200)	-- math.huge doesnt exist in lua 5.0
	for i=2, math.huge do
 	...
 	while not List.isempty(env.list) do	 		
		...
		local obj = List.pop(env.list)
 		local t = type(obj)
 		-- call the appropriate traverser for type
 		local gc = gc or _G
