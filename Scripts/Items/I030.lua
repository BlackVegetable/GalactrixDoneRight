-- I030
-- Platinum Transformer - Converts all yellow gems into red gems.

local function activate(item, world, player, obj,weapon,engine,cpu)
	local gemList = world:GetGemList("GENG")
	item:TransformGems(world, gemList, true, "GWEA")
end
...
local function should_ai_use_item(item, world, player)
	...
	if engine_need > (weapon_need + 5) then
		weight = math.random (1,55)
	elseif engine_need < (weapon_need - 5) then
	...
	if player:GetAttribute("weapon") >= (player:GetAttribute("weapon_max") - 1) then
		weight = 0
	end
