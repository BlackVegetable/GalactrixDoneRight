--
-- This file is autogenerated by QuestXml2Lua
--
require "quests/Quest" 


local quest = Quest {

   id                 = "Q0P1";
   log_text           = "[Q0P1_DESC]";
   icon               = "side";
   quest_type         = "side";
   repeatable         = false;
   can_abandon        = true;
   end_state          = 3;
   start_convo        = "Conv_Q0P1a";
   incomplete_convo   = "";
   level_min          = 1;
   level_max          = 999;
   start_message      = "";
   incomplete_message = "";
   start_locations = {S064=true,T237=true};

   preconditions = {
   };

   objectives = {
      [1]={
              OBJECTIVES.go_to_location {
                 location        = {S042=true,T114=true};
                 action			= "[Q0P1_1_0_ACTN]";
                 log_text		= "[Q0P1_1_0_LOG]";
                 end_text        = "[Q0P1_1_0_ENDMSG]";
                 next_state      = 2;
              },
           };
      [2]={
              OBJECTIVES.go_to_location {
                 location        = {S064=true,T237=true};
                 action			= "[Q0P1_2_0_ACTN]";
                 log_text		= "[Q0P1_2_0_LOG]";
                 next_state      = 3;
              },
          };
   };

   start_actions = {
   };

   end_actions = {
      [3]={
                 ACTIONS.give_faction_status {faction="12", amount=15, show=false},
                 ACTIONS.unlock_quest {quest="Q0P2", show=false},
                 ACTIONS.give_experience {amount=35, show=true},
          };
   };

   abandon_actions = {
   };

}

return quest
