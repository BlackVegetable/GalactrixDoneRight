...
function Hero:AddCombatStat(stat, bonus)
	if stat then
		...
		self.matchBonus = { 	weapon = math.floor(_G.GLOBAL_FUNCTIONS.DiminishingReturns(self:GetCombatStat("gunnery"),0.2)),
								...
								shield = math.floor(_G.GLOBAL_FUNCTIONS.DiminishingReturns(self:GetCombatStat("pilot"),0.2))
								}
	end
end
...
function Hero:OnEventDirectDamage(event)
    ...
    if damage > 0 then
		...
		e:SetAttribute("amount", damage)
		GameEventManager:Send(e, event:GetAttribute("source"))		
...
function Hero:OnEventShipDamage(event)
	...
	--[[
		...
		self:ReceiveDamage(amount, source)
	end
end

function Hero:ReceiveDamage(amount, src)
    local shield = self:GetAttribute('shield')
    local life = self:GetAttribute('life')
	local battleGround = SCREENS.GameMenu.world
	...
	local e = GameEventManager:Construct("DamageDealt")
	e:SetAttribute("amount", amount)
	GameEventManager:Send(e, src or self)
    ...
    if damage < 0 then
		...
		event:SetAttribute('source',src or self)		
...
function Hero:OnEventNewTurn(event)
	...
	--LOG("Mutate NewTurn x "..tostring(self:NumAttributes("Effects")))
	...
	if self:GetAttribute("ai") == 1 and battleGround then
		...
		for i=1, ship:NumAttributes("battle_items") do
			...
			if item:ItemActive(self:GetAttribute("engine"),self:GetAttribute("weapon"),self:GetAttribute("cpu")) then
...
function Hero:ActivateItem(battleGround,item)
	...
	local item_id = item.index
	if not item_id then
		--LOG("WARNING - item should have .index set in InitHeroBattleStats")
		for i=1,8 do
...
function Hero:PopupPerformingAction(title,message,callback)
	--LOG(string.format("Hero:PopupPerformingAction(%s,%s)",title,message))
	self:QuestPopup(title,message,callback)	
end

function Hero:PopupObjectiveComplete(title,message,callback)
	--LOG(string.format("Hero:PopupObjectiveComplete(%s,%s)",title,message))
	self:QuestPopup(title,message,callback)
end
	
	
function Hero:PopupArriveAtLocation(title,message,callback)
	--LOG(string.format("Hero:PopupArriveAtLocation(%s,%s)",title,message))
	self:QuestPopup(title,message,callback)	
end

	
function Hero:QuestPopup(title,message,callback)
	LOG("QuestPopup")
	if is_message_menu_open() then
		close_message_menu(true)--call any quest message callback--queuing system is needed
	end
	if _G.is_open("SolarSystemMenu") then--this should always be open, but we'll check anyway
		SCREENS.SolarSystemMenu.state = _G.STATE_MENU
	end
	open_message_menu(title,message,callback)
end
