...
local function createBeam(beamObj,world,startx,starty,endx,endy)
	...
	local beam = create_beam("Assets/Beams/intel.png", 0, 700, my_callback, 10)
	beam:begin_update()
	...
	AttachParticles(world, "WhiteBeam", world:ScreenToWorld(startx,starty))	
	AttachParticles(world, "WhiteBeam", world:ScreenToWorld(endx,endy))	
	
	BEAMS["BM13"]:CreateBeam(world,startx,starty,endx+math.random(-10,10),endy+math.random(-10,10))
end
