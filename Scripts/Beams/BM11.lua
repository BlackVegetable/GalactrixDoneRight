...
local function createBeam(beamObj,world,startx,starty,endx,endy)
	...
	local beam = create_beam("Assets/Beams/psi.png", 0, 800, my_callback, 20)
	beam:begin_update()
	...
	AttachParticles(world, "PurpleBeam", world:ScreenToWorld(startx,starty))
	AttachParticles(world, "PurpleBeam", world:ScreenToWorld(endx,endy))
			
	BEAMS["BM10"]:CreateBeam(world,startx,starty,endx+math.random(-10,10),endy+math.random(-10,10))
end
